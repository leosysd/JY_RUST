...
const TUNABLE_PARAMS: &[(&str, &str, &str)] = &[
    ...
    ("ACCUM_RESCUE_MIN_SECONDS_LEFT", "accum 补救最晚进场线(剩余≤此秒不救,0=关闭)", "0"),
    ("ACCUM_RESCUE_MAX_SHARES", "accum 补救每盘最大补救份额(0=不限)", "0"),
    ("ACCUM_RESCUE_MAX_WORST_LOSS", "accum 补救押错最大真实亏损(0=关闭)", "0"),
    ("ACCUM_LATE_CASCADE_ENABLED", "accum 严格2:1 late-confirm(1开/0关)", "0"),
    ("ACCUM_LATE_CASCADE_QTY", "late-confirm 主边目标份额", "295"),
    ("ACCUM_LATE_CASCADE_HEDGE_FRAC", "late-confirm 对冲比例(0.5=最大2:1)", "0.5"),
    ("ACCUM_LATE_CASCADE_MAX_EXPOSURE", "late-confirm 单盘最大含费成本", "300"),
